...
**Follow-ups:** see in order — [`path-a5-socketinfo-construction.md`](./path-a5-socketinfo-construction.md), [`path-a6-plug-slot-origin.md`](./path-a6-plug-slot-origin.md), [`path-a7-cons-construction.md`](./path-a7-cons-construction.md), [`path-a8-sraw-handler.md`](./path-a8-sraw-handler.md), [`path-a9-sraw-outbound.md`](./path-a9-sraw-outbound.md).

> **CALIBRATION 2026-04-21 (per Codex parity #315 idx 4):** This doc's $106e interpretation framed it as a "THINK C shared-entry — no installer." **That interpretation is WRONG** and is revised by Path A.9 (2026-04-21). What is MEASURED: the bytes `60 00 00 f0` at 0x106e are a `BRA.W +0xf0` (→ 0x1160, still inside SendData). What is OVERRULED: the conclusion "therefore no install needed" — Path A.9 shows that if 0x106e truly acted as a BRA-to-epilogue, SendData would never emit CDB bytes (the 0x1160 code is the reply epilogue, not a sender). The SRAW handler's 7-arg push at file 0x0f4a-0x0f5e exactly matches `SMSendData`'s (file 0x160c) calling convention, and SMSendData builds the real CDB. **Revised reading (CANDIDATE, strong):** 0x106e IS a runtime-patchable slot; 2-byte displacement at 0x1070-0x1071 is rewritten at load time so BRA.W targets SMSendData instead of 0x1160. Patcher identity is OPEN (task #35 / Path A.11). The frame about the slot fn_ptr at $11fe (REPLY direction) is a SEPARATE mechanism and remains CANDIDATE until A.10 settles the editor-side packing step.
