...
| Phase 3: Bridge Implementation | **Option 2 committed; reply path MEASURED, install-edge identity CANDIDATE, outbound CDB OPEN** | Calibration adopted from Codex parity (#315 idx 4): claims tagged MEASURED / CANDIDATE / OPEN. **MEASURED:** plug-side `CONS → ConnectToSocket` handler; `ASOK → ActivateSocket`; `plug_slot[+0] ← SocketInfo[+0]` copy in plug's `ConnectToSocket`; the full REPLY chain (A.8) `main → vtable[+0xA8] dispatcher → ADAT handler → CMESASocket::AcceptData` (`_BlockMove` reply bytes, store 'SRAW'/'SYSX' at `CMESASocket[+12]`). **CANDIDATE (not PROVED):** `0x212 → file 0x028169 (main)` as the editor-side callback identity; `editor[+0x8c] = CMESASocket[+24] = SocketInfo[+0]`. The byte-level decode is solid; the link from "ctor sets this field" to "plug receives this exact byte at CONS time" remains undecoded. **OPEN:** (a) outbound CDB emission for SRAW audio uploads in `CSCSIPlug::SendData` SRAW handler (file 0x0ec0, before shared-entry JSR; task #33 = Path A.9 — directly answers original Phase 3 question); (b) editor→plug SocketInfo transmission/packing step (task #34 = Path A.10; closes Codex pressure point — likely in-process function call, not SCSI wire). Earlier "external runtime patching" hypothesis is dissolved either way. Community writeup planned as terminal deliverable. |
