...
| Phase 3: Bridge Implementation | **Option 2 committed (harness end-to-end via terrain-as-necessary)** | Static decode is exhausted: (task #30 + Codex parity) no install edge for SCSI Plug's $106e sender inside either binary we have; SRAW flag consumed entirely by externally-installed function. Per `decision-record-2026-04-19.md`: commit to extending `mesa-plug-harness` to load and execute `sampler-editor-rsrc.bin`, building Mac toolbox stubs incrementally per Codex's priority test plan (P1→P4). Path A (task #31) is step 0 — static decode of the install-edge path for install-target identification before harness bootstrap. Effort range: 3-6 weeks per the `#315` analysis, mitigated by terrain-as-necessary scope control. Community writeup planned as terminal deliverable. |
