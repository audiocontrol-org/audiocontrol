...
| Phase 3: Bridge Implementation | **🎯 PRIMARY RE GOAL ANSWERED 2026-04-23/24** | **Full transport path captured end-to-end in pure emulation** under bounded, evidence-backed harness shortcuts. **MEASURED real CDB family emitted by MESA II's plug for sample data transfer:** `0x0C 00 <len24> <mode>` where mode = `0x80` for SRAW payload, `0x00` for BULK opener/continuation; WRITE direction; target = chooser-selected device. Three shortcuts stand in for known production stages: `this+0x0d72` (ChooseSCSI success-tail publication), `this+0x0e40 = 1` (SendData precondition gate), `BYPASS_1187E` (chooser-completion contract). Full lifecycle: INIT → CONS → ASOK → ChooseSCSI-published-state → SEND command → SendData → SMSendData → local CDB build → PB/SCSI Mgr 4.3 helper → `$A089 SCSIDispatch`. **Bridge parity divergence discovered:** current `scsi_midi_send()` hardcodes CDB[5]=0x00, but MESA II SRAW phase uses 0x80. May explain long-standing SLNGTH slow-upload. **Division of labor:** Claude owned emulator-forward execution; Codex owned static decode + contract identification + bridge-side parity check. See [#315](https://github.com/audiocontrol-org/audiocontrol/issues/315) for the running joint log (100+ comments over two sessions). Next: hardware acceptance test (CDB[5]=0x80 on S3000XL) as the concrete bridge mismatch test. |
