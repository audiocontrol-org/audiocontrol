...
| Phase 3: Bridge Implementation | **Option 2 committed; install-edge static decode CLOSED (#31 done)** | Path A → A.5 → A.6 → A.7 fully decoded the install edge: editor ctor (file 0x0596e7) installs fn_ptr 0x212 into `SocketInfo[+0]`; CONS SCSI command transmits SocketInfo to plug; plug's `ConnectToSocket` (file 0x09d2) copies into `plug_slot[+0]`; `$11fe` JSRs there, into editor's `main` callback at file 0x028169. `main` is a generic dispatcher with one INIT-magic check; everything else dispatches via `vtable[+0xA8]` of a global handle constructed by `JSR $0x272A4` at INIT time. **Earlier "external runtime patching" hypothesis is dissolved** — install is either compile-time (vtables, static data) or a defined wire transaction (CONS payload). Materially reduces Option 2 harness scope. Optional Path A.8 (task #32) decodes the constructor + vtable[+0xA8] to identify the actual SRAW data handler. Otherwise proceed to P1 harness terrain. Community writeup planned as terminal deliverable. |
