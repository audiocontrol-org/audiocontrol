...
- [x] ~~Decode CMESASocket::vtable[0x38] (task #19)~~ — done. Was a red herring — it's actually `CAkaiSampler::vtable[0x38]` = `SwapLongWord` (32-bit byte reversal). The 6 vtable[0x38] calls in BuildSampleHeaderFromMAH dispatch through `CSamplerModule@(0xDA4)` = `CAkaiSampler*`, not a separate CMESASocket. Test updated with `swapLongWord()` encoding. See `mesa-ii-analysis/cmesasocket-vtable38-decoded.md`.
- [ ] **Preamble hypothesis (task #20)**: BULK test still fails despite byte-perfect encoding. Hypothesis: SDATA opcode 0x0B is an UPDATE command requiring the sample slot to pre-exist. MESA's SCSI flow sends a preamble SDS dump header via vtable[0x0030] before the BULK loop. Either (A) test preamble directly by adding standard SDS create to the test, or (B) decode vtable[0x0030] to see what it emits.
