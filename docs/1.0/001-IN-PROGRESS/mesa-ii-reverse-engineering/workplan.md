...
- [ ] **Replicate ActivateThisSocket sequence (task #21)**: decode the conditional JSR at file 0x05a0f9 (function pointer at this+52+(48*this+2474)) to determine if ActivateThisSocket emits any wire bytes. If no, the BULK test approach can't work without driving end-to-end via the harness. If yes, replicate those bytes in the test.
- [ ] **Strategic decision (post-task-#21)**: continue down the decode rabbit hole, expand harness to drive full SendAudioBufferToSampler end-to-end (loading sampler-editor binary alongside scsi-plug binary), or pivot to alternate fast-upload approach (e.g., scan ASPACK opcode neighbors for "create-with-real-length" command, or maximize SDS batch throughput).
