...
| Phase 9: LCXL3 DAW Mixer + Plugin Control | PR Open ([#357](https://github.com/audiocontrol-org/audiocontrol/pull/357)) | 9a (research + profiling), 9b (mixer mode implementation: faders → channel volume, V-pots → pan, fader buttons → mute/solo/arm/select with row toggles, banking, LED feedback), 9c (mode-entry buttons → LUNA Plug-In/Sends/Cue) all on branch and hardware-tested. 9d formal validation pass deferred. |
| Phase 10: LCXL3 Page-Aware V-Pot Mapping | PR Open ([#357](https://github.com/audiocontrol-org/audiocontrol/pull/357)) — scaffolding only | Page 0 default = Row 1 Trim, Row 2 Tape saturation; Pages 1-4 = paired Sends; bottom row always Pan. DAW Control V-pots = EQ / focused-plugin per LCXL3 reference. Single LCXL3 LCD mirrors LUNA's parameter-display SysEx (Live/Logic style). `vpot_page` state scaffold + Page Up/Down handlers + Shift+Track ↔ Track ◀/▶ disambiguation landed; full routing requires Phase 10a drill-down profiling + 10b state machine + 10c hardware validation. |
...
- Open Issue (deferred): [#356](https://github.com/audiocontrol-org/audiocontrol/issues/356) — Track ◀/▶ ignored by LUNA
- Phase 9 + Phase 10 scaffold Pull Request: [#357](https://github.com/audiocontrol-org/audiocontrol/pull/357) (open)
- Milestone: TBD
