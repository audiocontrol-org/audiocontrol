...
| Ableton parser fixes | Complete (uncommitted) | Multi-message MIDI splitter in `midi.rs` plus BBT separator-bit mask in `mcu.rs::DigitChar::from_byte`. Ableton bundles 10+ messages per CoreMIDI packet and uses `0x70-0x79` for "digit + dot" — both prevented its position CCs from being parsed. Tests added; bridge correctly tracks Ableton's playhead. Commit pending. |
| Phase 5: LCXL3 multi-input | Complete | LCXL3 as a second input source alongside MC-500. Sub-phases 5a (lcxl3 protocol module), 5b (state-machine variants), 5c (config), 5d (main wiring), 5e (hardware validation) all merged on `feature/midi-macro-bridge-ableton`. Hardware-validated 2026-04-27: Play/Stop toggle drives LUNA, jog encoder nudges bars, LED follows state, encoder during playback ignored, sync-on-stop still fires. 122 unit tests passing. PR-ready. |
