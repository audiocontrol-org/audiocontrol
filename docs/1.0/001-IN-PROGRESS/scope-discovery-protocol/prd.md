...
5. **Inventory artifacts on disk — manifest canonical, discovery evidence retained and journaled.** The per-feature `scope-manifest.yaml` lives **in the feature docs directory** alongside `prd.md` and `workplan.md` (e.g., `docs/1.0/001-IN-PROGRESS/<feature-slug>/scope-manifest.yaml`) — committed, review-visible, lifecycle-tracked. Hiding the manifest in a gitignored location would invite drift between the manifest and the rest of the feature documentation. **Discovery evidence is also committed, not ephemeral.** The discovery artifacts (screenshots, DOM-token snapshots, per-agent raw findings JSON, synthesis-pass narrative) live at `docs/<version>/<status>/<feature-slug>/scope-inventory/`, organized as timestamped per-run subdirectories with a `journal.md` index narrating each run. The evidence trail is load-bearing — it's how a future session reconstructs *why* the manifest names what it names, and how scope findings correlate with design and implementation decisions. Discovery captures are not reproducible in any meaningful sense (they're a snapshot of repo state at discovery time); treating them as ephemeral would discard the audit trail. Re-invocations of `/scope-inventory` append new run directories rather than overwriting. The repo-level `clones.yaml` lives at `docs/scope-discovery/clones.yaml` — committed; the operator-disposition column is part of the project's review record. Full layout contract — directory shape, per-run subdirectory contents, naming conventions, and the git-ignore invariant — lives at [`../../../scope-discovery/LAYOUT.md`](../../../scope-discovery/LAYOUT.md).
6. **Validation by drain.** The audiocontrol repo's existing duplication backlog IS the validation case. The feature is not done until `docs/scope-discovery/clones.yaml` has zero un-dispositioned entries and every `refactor`-marked entry has a merged PR.
...
- [ ] **Validation by drain:** the tooling has been run against `modules/*/src/` in the audiocontrol repo; `docs/scope-discovery/clones.yaml` exists with every detected clone group dispositioned as `refactor | keep-with-reason | ignore-with-justification`. Zero un-dispositioned entries remain. Every `refactor`-marked entry has a merged PR. Every `keep-with-reason` and `ignore-with-justification` entry has a one-line justification committed alongside.
- [ ] Per-feature `scope-manifest.yaml` is committed alongside `prd.md` and `workplan.md`; per-feature `scope-inventory/` directory is committed (timestamped per-run subdirectories holding per-agent findings JSON, captures, and synthesis narrative; `journal.md` index narrating every run); repo-level `clones.yaml` is committed at `docs/scope-discovery/clones.yaml`. Re-running `/scope-inventory` appends a new run directory rather than overwriting; the journal records each invocation with a date stamp and a short narrative. Nothing under the feature docs tree is gitignored.
...
- **Discovery-evidence repo size growth.** Per-feature `scope-inventory/runs/` directories accumulate screenshots and DOM-token JSON — roughly 2-6 MB per typical UI run (~20 captures at 100-300 KB each), accumulating across N system-wide features × M re-invocations each. The operator's framing — *"discovery artifacts MUST be retained for traceability"* — accepts this cost as the price of the audit trail. Mitigations: (a) DOM-token JSON gzipped at write time; (b) PNG-8 with quantization where it doesn't compromise legibility; (c) at feature close, runs older than the final converged run can be operator-pruned to a representative subset with the prune action logged in `journal.md`. Pruning is operator-initiated, never automatic. If size becomes a real problem, git-lfs is the follow-up; out of scope for v1.
