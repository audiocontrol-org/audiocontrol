...
| Phase 5 — Refactor Preconditions (CRITICAL) | Planning | [#443](https://github.com/audiocontrol-org/audiocontrol/issues/443) | Refactor commits require declared canonical side + proven regression-detection coverage before they can land. Addresses the MUST FIX from `feature/roland-bugfix`'s tooling feedback (regime-erasure prevention) + the operator-added test-precondition rule. **Must land before scaling clone-driven refactor work to other branches.** |
| Phase 6 — Regime-Holdout Discovery | Planning | [#444](https://github.com/audiocontrol-org/audiocontrol/issues/444) | Anti-pattern registry + adopter manifests + cross-editor symmetry checker + deprecation-driven scan + new `regime-holdout-detector` agent joining `/scope-inventory`'s fleet. Catches the regime drift that doesn't show up as clones. |
| Phase 7 — Tooling Hardening + Operator QoL | Planning | [#445](https://github.com/audiocontrol-org/audiocontrol/issues/445) | Content-hashed clone-group IDs (no more line-shift orphaning), self-installing `make scope-inventory`, `make clone-summary`, upstreamed `batch-dispose.ts`, polish bundle. |
