...
Resolution:
- Diagnosis: Hypothesis 2-variant. `mergeDispositions` itself is correct (basic happy path scenarios 1+2 confirm content-hash-match preserves both keep-with-reason and refactor dispositions byte-for-byte across `--refresh-baseline`, default mode, and 5 consecutive refresh passes). The silent-loss path is one layer up: `clones-yaml.ts:parseClonesYaml` returns `null` on ANY shape error (e.g., an entry missing a required field like `members`, an unrecognized `disposition` value, malformed YAML) and `clone-detector.ts:readBaseline` mapped that `null` to `treating as empty baseline` (stderr warning + proceed). When followed by a `--refresh-baseline` write — exactly the path `make refresh-clones-baseline` triggers — the detector wrote ONLY the freshly-detected groups (all `pending+null`) and silently destroyed every operator disposition. The subsequent pre-commit compare against the refreshed baseline reports `0 NEW, 0 DROPPED` because the baseline now matches detected state — matching the operator's repro exactly. Synthetic fixture confirmed: pre-change code, planted baseline with one `keep-with-reason` entry + one shape-broken sibling, `--refresh-baseline` → exit 0, baseline overwritten with `pending+null` only. Side-test confirmed jscpd's fragment text is deterministic across runs (no fragment-instability path) and T7.1 content-hashed ids are stable across adjacent-file line-shifts (no hash-drift path).
- Light fix: the parse layer was extracted into a new sibling `tools/scope-discovery/clones-yaml.parse.ts` (host `clones-yaml.ts` was at 563 lines after the strict-parse expansion; the split keeps both files under the 300-500 cap and matches the existing `clones-yaml.id.ts` / `clones-yaml.refactor.ts` SSOT pattern). The parse layer now exposes three entry points: `parseClonesYaml` (lenient, returns null on shape error — for callsites that legitimately treat malformed YAML as "no baseline," e.g., the T7.1 migration script's pre-migration read); `parseClonesYamlStrict` (throws `ClonesYamlParseError` with a per-entry / per-field reason); and `parseClonesYamlDetailed` (structured `{ ok, doc | reason }` — the other two are thin adapters). `clone-detector.ts:readBaseline` now uses the strict variant; an existing-but-malformed baseline triggers exit 2 with the actionable message `baseline <path> exists but is malformed: clones[<idx>]: id=<id>: <field> missing or not a sequence ... Refusing to silently overwrite operator-curated dispositions.` The on-disk baseline is preserved byte-for-byte in the error path. `RefactorPreconditionError` propagation is unchanged.
- Heavy backstop: `tools/scope-discovery/check-disposition-survivor.ts` (new) is a pre-commit gate that compares `git show HEAD:docs/scope-discovery/clones.yaml` against `git show :docs/scope-discovery/clones.yaml` (the staged blob). For every HEAD entry whose `disposition ∈ {keep-with-reason, ignore-with-justification, refactor}`, the gate fails if the same `id` appears in the staged version with `disposition: pending`. Other transitions are not flagged: id-removed (legitimate refactor / membership-change drop) is visible in the standard pre-commit clone diff; protected → protected (e.g., `keep-with-reason` → `ignore-with-justification`) is a reclassification, not a loss; brand-new pending entries are fresh detections, not transitions. Operator override via `--force` (loud stderr warning naming every accepted id+transition). The Makefile target is `make check-disposition-survivor`; pre-commit wires it in via a new `HAS_CLONES_YAML` staged-file sniffer that mirrors the existing `HAS_TS` / `HAS_CSS` shape, so the gate is dormant for the common-case commit (most commits don't touch `clones.yaml` and pay zero cost). Catches THIS bug (silent-loss after a malformed-baseline regen) AND every future regression of the same shape regardless of root cause — the architectural floor for the disposition-survival contract.
- Adversarial validator: `tools/scope-discovery/disposition-survivor.validate.ts` exercises 8 scenarios (the 5 Heavy-gate scenarios live in a sibling `disposition-survivor.gate-scenarios.ts` per the file-cap split; the host owns the 3 Light-fix scenarios + the scheduler). (1) `content-hash-match preserves keep-with-reason + multi-paragraph reason` — synthetic baseline with the operator's verbatim reason shape; `--refresh-baseline`; assert disposition + reason survive byte-for-byte. (2) `content-hash-match preserves refactor + canonical fields` — synthetic refactor entry with `canonical_side: new` + `new_shape_summary` + `tests` + `tests_proof`; `--refresh-baseline`; assert all five precondition fields survive. (3) `malformed-baseline fails loud (exit 2) and does NOT silently wipe dispositions` — synthetic baseline with one `keep-with-reason` entry + one shape-broken sibling (missing `members`); `--refresh-baseline`; assert exit 2 + stderr names "malformed" + "Refusing to silently overwrite" + on-disk file unchanged byte-for-byte. (4) `disposition-survivor gate blocks keep-with-reason → pending transition` — throwaway git fixture; HEAD has `keep-with-reason`; staged has same id with `pending`; assert exit 1 + stderr names the id + transition + `--force` override path. (5) `disposition-survivor gate allows new pending entries (no loss diff)` — staged adds a fresh `pending` entry (no transition); assert exit 0. (6) `disposition-survivor gate allows protected → protected transitions` — staged changes `keep-with-reason` → `ignore-with-justification`; assert exit 0. (7) `disposition-survivor gate --force override accepts loss diff` — same fixture as scenario 4 + `--force`; assert exit 0 + stderr acknowledges override + names overridden id. (8) `gutted-stub self-check: a no-op gate MUST fail scenario 4` — a stub that always exits 0 with no output is rejected by scenario-4's id-in-stderr assertion (confirms harness teeth). Wired via `make check-disposition-survivor-validate` and into `pnpm test:scope-discovery` (232 → 240 aggregate scenarios across 18 validator suites).
- **AUDIT-07 hard test outcome (validator-paired changes):** stashed every production-code file (`clones-yaml.ts`, `clone-detector.ts`) and moved the new gate file (`check-disposition-survivor.ts`) aside so production code matched the pre-change state. Kept the new scenarios module + Makefile wiring + pre-commit hook changes in place. Re-ran `tsx tools/scope-discovery/disposition-survivor.validate.ts`. Result: 5 of 8 scenarios FAIL — scenario 3 (`malformed-baseline fails loud`) got exit 0 instead of 2 (Light fix gone, silent-wipe path active); scenarios 4-7 all failed with `Cannot find module '.../check-disposition-survivor.ts'` because the gate file was moved aside. The 3 PASSING scenarios are correctly regression-guards: scenarios 1+2 assert the happy path (content-hash-match preservation), which works under both pre- AND post-change `mergeDispositions` because that function was always correct — those scenarios protect against a future regression in the happy path; scenario 8 (gutted-stub self-check) asserts that an always-pass stub gate fails scenario-4's id-in-stderr assertion, which is true against both pre- and post-change code by design. After `git stash pop` + restoring the gate file, all 8 scenarios pass. Confirms every load-bearing scenario has teeth + the three regression-guards are correctly shaped.
- Empirical re-exercise (2026-05-24): synthetic post-change repro (plant baseline with one `keep-with-reason` entry + one shape-broken sibling missing `members`; run `tsx tools/scope-discovery/clone-detector.ts --refresh-baseline`) reports exit 2 with stderr `baseline ... exists but is malformed: clones[1]: id=brokenentry01: \`members\` missing or not a sequence ... Refusing to silently overwrite operator-curated dispositions.` and the on-disk baseline file unchanged byte-for-byte (both the good `keep-with-reason` entry and the broken sibling preserved exactly as planted). Heavy gate synthetic repro (throwaway git repo; HEAD commit with `keep-with-reason` + multi-paragraph reason; stage a version that reverts to `pending`; run `tsx tools/scope-discovery/check-disposition-survivor.ts`) reports exit 1 with stderr naming the id, the transition direction, the first member, the first line of the HEAD reason, and the `--force` override path. The live tree's `clones.yaml` is not staged on this branch so the gate exits 0 silently in the normal pre-commit path (verified by direct invocation). Side-effect checks: `pnpm test:scope-discovery` reports `240/240` scenarios passing across 18 validator suites (was 232 — +8 from `disposition-survivor.validate.ts`); `tsx tools/scope-discovery/clone-detector.ts --quiet` reports `457 groups; 0 NEW; 0 DROPPED`; `pnpm exec tsc --noEmit` exits 0. Pre-commit hook recorded the fix commit in `.git/hooks-sentinels/.pre-commit-passed`; no hooks bypassed.
