...
| Phase | Status | Notes |
|-------|--------|-------|
| Phase 1: Baseline and Comparison Setup | In Progress | Claude branch baseline captured; comparison artifacts created; first Codex target selected |
| Phase 2: Independent Codex Analysis | Not Started | Produce Codex-led evidence-backed findings |
...
- the remaining blocker is the content encoding of the 200-byte Akai header,
  especially the fields written through `CMESASocket::vtable[0x38]`
...
identifies as its blocking unknown: `CMESASocket::vtable[0x38]` and the exact encoding
of the header fields at offsets 26-47.
