...
- [ ] **1.1 Inventory akai pages + test-harness coverage.** Walk `modules/akai-s3k-editor/src/pages/` and identify every page. For each, confirm whether a `Test<PageName>Page.tsx` (or equivalent harness route) exists. If a page lacks a harness route, add one before the audit can iterate against it (per the established convention: API tests are not UI tests; browser UI needs Playwright tests).
  ...
  - **Partial:** the Keygroups route still points at the pre-existing inline-styled harness (kept for `zone-overview.spec.ts` continuity, which depends on it). The Library route mounts a stub `<div>` standing in for `PluginLibraryBrowser`. AUDIT-20260524-06 + -07 flagged both gaps. Re-closing the task requires landing a shell-compliant `TestKeygroupsShellPage` at a new route (so `zone-overview.spec.ts`'s `/test/keygroups` stays intact) AND a deterministic real-`PluginLibraryBrowser` harness (or paired spec) that exercises the production browser's overflow ownership.
