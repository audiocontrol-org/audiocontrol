...
- [ ] **2.2 Per-primitive migration commits.** One commit per primitive in `harmonization-spec.md`, in dependency order (lowest-level primitives first, page-level shells last). For each:
  ...
  - **STILL OPEN under task 2.2**: AcRangeBar (extract from VelocityRangeBar / VelocityZoneEditor inline SVG); AcEnvelope (parameterize segment count); AcChevron (canonical primitive landed in `feature/roland-bugfix`; akai-side adoption); page-level shells (Live-editing footer / Virtual front panel / fixed-viewport shell). The S3kParamRow `tooltip:` prop currently throws when supplied — wiring requires the akai Tooltip primitive to land (separate sub-task).
