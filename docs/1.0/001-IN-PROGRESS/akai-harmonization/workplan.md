...
- [ ] **1.4 Run the `/frontend-design` audit** comparing each akai page to its roland equivalent. Produce per-page sections in `harmonization-spec.md` listing every visible primitive (page header, list rows, drawers, dialogs, chrome bands, eyebrow rows, status footers, virtual front panel, etc.) and its current state in each editor.
- [ ] **1.5 Disposition every primitive in the spec.** For each primitive, record one of:
  ...
  Document the rationale per disposition. The disposition entry is the proof-of-design for the `canonical_side` declaration in the Phase 2 refactor commit that implements it.
- [ ] **1.6 Identify akai-specific anti-patterns** surfaced by the audit. Add to a Phase 1 deliverable list of anti-patterns to register in Phase 2 against `docs/scope-discovery/anti-patterns.yaml`.
- [ ] **1.7 Identify scope-discovery-protocol tooling gaps.** If the audit surfaces gaps (e.g., the editor-symmetry matrix needs to accept akai as a participant, or a registry schema field is missing), file them as issues against the scope-discovery-protocol feature for backfill during Phase 2.
