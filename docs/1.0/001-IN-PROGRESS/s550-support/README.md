...
**Status:** In Progress. **Phase 9 REOPENED 2026-05-13 — false closure.** Phases 1-8, 10 Complete; Phase 7 Complete (2026-05-12); Phase 0 Task 10 Complete (all 7 waves).
...
| **Phase 9 — UX/UI Cleanup REOPENED 2026-05-13 — FALSE CLOSURE.** | Tasks 1-7 previously claimed complete 2026-05-12 are INVALIDATED. Live-hardware testing 2026-05-13 confirmed every parameter slider is non-functional ([#424](https://github.com/audiocontrol-org/audiocontrol/issues/424)) and PlayPage retains the legacy sticky page header ([#423](https://github.com/audiocontrol-org/audiocontrol/issues/423)). The 175-passing capability suite drives writes via `.fill()` on internal number-inputs, not via pointer/keyboard events on the visible affordances — it verifies the wiring seam, not the UI. Remediation plan supersedes the previous Tasks 4-7: **9R-A** test-strategy reset (codify wiring-vs-UI test distinction, move existing capability specs to `test/wiring/`, demote screenshot spec to rendering smoke) → **9R-B** primitive remediation (`AcRangeBar` / `AcSlider` / `AcCheckbox` / `AcSelect` / `AcNumberInput` / `AcEnvelope` get real pointer+keyboard handlers + interactive contract tests + accessible ARIA shapes) → **9R-C** page rebuild (every page operator-driven on real hardware with pointer/keyboard-event Playwright specs) → **9R-D** operator holistic hardware closure gate. Phase 9 is not closed until the operator can verbatim record *"Phase 9 closed; the redesign delivers what was asked for"* in DEVELOPMENT-NOTES.md. See [workplan.md](./workplan.md) Phase 9 section for full plan. | — |
