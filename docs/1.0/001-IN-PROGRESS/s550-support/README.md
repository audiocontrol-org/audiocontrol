...
> **⚠ 2026-05-17 — Capabilities inventory + 9R remediation MOTHBALLED.** The capability documents (`ROLAND-S550-EDITOR-CAPABILITIES.md` + `ROLAND-S550-EDITOR-CAPABILITIES-DETAILED.md`) and the Phase 9 Remediation Plan (9R-A / 9R-B / 9R-C / 9R-D) in [`workplan.md`](./workplan.md) are paused. Operator decision: post-redesign regressions are fixed directly, in real time, without routing through the four-tier coverage / sign-off process. Mothball banners on the affected docs are authoritative; treat row-level statuses inside them as historical. Current work: direct regression fixes against the redesigned editor.
...
- Everyone: use `ROLAND-S550-EDITOR-CAPABILITIES-DETAILED.md` as the historical product contract for what the UI was supposed to do during the mothballed capabilities phase; current work is direct regression fixing against the redesigned editor.
...
| Phase 11 — Cross-Cutting Quality Audit Items | **Task 1:** [#425](https://github.com/audiocontrol-org/audiocontrol/issues/425) ImportSamplesDialog slot-occupancy mislabel — surfaced 2026-05-14 by independent audit; canonical helpers exist but unused at three sites in the dialog. **Task 2:** [#424](https://github.com/audiocontrol-org/audiocontrol/issues/424) primitive remediation sweep — partially closed via [`406dc1e7`](https://github.com/audiocontrol-org/audiocontrol/commit/406dc1e7) (AcRangeBar + AcEnvelopeTable); remaining primitives (AcSelect / AcCheckbox / AcNumberInput / AcSlider / AcEnvelopeGraph / AcEnvelopeMeta) are 9R-B scope, surfaced here for session-resumption visibility. **Task 3:** [#426](https://github.com/audiocontrol-org/audiocontrol/issues/426) root `test/ui/*.spec.ts` test-discipline gap — **CLOSED 2026-05-15** via mixed Option B + Option C (commit `215308b5` + SHA fix-up `f59b7ea9`); auditor verified `verified-2026-05-16`. Option C (delete) for `library/patches/play/tones.spec.ts`; Option C with migration for `home.spec.ts` (Continue-to-Patches assertion → wiring `C-CONN-02b`); Option B for `page-viewport-containment` + `phase-7-task-2-front-panel-screenshots` (moved to `test/rendering/`). ESLint scope widened to `**/test/ui/**`. Counts: wiring 136→137, ui 26→6, rendering 14→24. **Task 4:** [#427](https://github.com/audiocontrol-org/audiocontrol/issues/427) live S-550 conformance suite — **AUDITOR-OWNED** per the 2026-05-15 protocol clarification; controller responsibilities limited to (1) maintaining the `BrokenContextWrapper` production-page infra and (2) remediating findings the suite surfaces. Auditor-landed specs: `s550-play.design.spec.ts`, `s550-patches.design.spec.ts`, `s550-D-TONE-live-envelope-and-slider.spec.ts`, `s550-library.design.spec.ts`. Findings surfaced so far: `LIVE-S550-PLAY-001` (→ #423), `LIVE-S550-TONES-001` (→ #428), `LIVE-S550-LIB-001` (→ #429). **Task 5:** [#429](https://github.com/audiocontrol-org/audiocontrol/issues/429) SaveSetDialog + library-dialog family missing `Dialog.Description` — surfaced 2026-05-15 by `LIVE-S550-LIB-001`; Radix accessibility warning on every dialog open. **Task 6:** [#430](https://github.com/audiocontrol-org/audiocontrol/issues/430) + [#431](https://github.com/audiocontrol-org/audiocontrol/issues/431) live S-550 `RQD` / stale-`RJC` defect class — partial closure: **#431 closed `verified-2026-05-16`** (both bounded patch assertions passed on live hardware after diagnostic instrumentation commit `9d1166a0`); #430 still `acknowledged` with updated manifestation evidence ("Wave data request rejected" on tone-0 save, same protocol-layer defect class). New: [#432](https://github.com/audiocontrol-org/audiocontrol/issues/432) `LIVE-S550-TONES-002` — bounded live Tones battery fails inside the editor (cutoff write-readback mismatch delta 38 + TVA sustain pointer stall); `acknowledged-#432` routed to workplan §9R-C natural-fit (NOT a new Phase 11 task — verification signal on already-in-flight 9R-C TonesPage rebuild). **Task 7:** operator runbook execution layer — **AUDITOR-OWNED**, new 2026-05-16 scope to convert the dated operator review runbook into a non-mutating integration path that reuses the live conformance suite instead of duplicating it. See [workplan.md](./workplan.md) §Phase 11 and [Live S-550 Conformance Matrix](./live-s550-conformance-matrix.md). | Task 1: independent (parallel with 9R-A.2). Task 2: 9R-A.1 infrastructure (DONE) — feeds into 9R-B. Task 3: independent; cleanest landing alongside 9R-A.4. Task 4: auditor-owned suite; controller-side infra DONE (via Task 1 commit `12ef2c18`); remediations flow through normal protocol loop. Task 5: independent; family-wide accessibility audit. Task 6: shared protocol-timing investigation on real hardware. Task 7: auditor-owned runbook-orchestration layer; must stay non-mutating in the shared worktree and reuse Task 4 specs. |
