...
|-----------|-------------|------------|
| **Phase 0: Frontend/Backend Decoupling** — All Tasks Done | All 9 tasks shipped: contract audit, fixture schema, `RecordingProxyAdapter`, CLI runner, 4 captured S-330 fixtures (incl. 1136-record `load-everything`), `SimulatedAdapter`, `?midi=simulated` URL-param mode + Vite fixture middleware (Task 7), 9 passing Playwright UI specs at `roland-sxx0-editor/test/ui/` (Task 8), CI workflow + drift-detection script (Task 9). Deferred follow-ups tracked in [#404](https://github.com/audiocontrol-org/audiocontrol/issues/404) (targeted fixtures), [#405](https://github.com/audiocontrol-org/audiocontrol/issues/405) (PatchesPage tone-preload decoupling), [#406](https://github.com/audiocontrol-org/audiocontrol/issues/406) (pre-existing CI test failures). See [phase-0-decoupling.md](./phase-0-decoupling.md) + [contract audit](./phase-0-contract-audit.md). | None — Phase 9 / Phase 7 unblocked |
