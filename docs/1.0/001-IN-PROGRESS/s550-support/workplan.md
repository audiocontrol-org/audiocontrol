...
- [ ] The Tier 3 dependency-wiring smoke proof from runbook §1.4 runs only in a temp copy / throwaway worktree and never mutates the shared active worktree. (Helper landed via `scripts/runbook-tier3-drop-smoke.ts` plus `runbook-tier3-drop-smoke.integration.test.ts`; current branch state intentionally blocks execution until `D-TONE-ENV-02` reaches `coverage=confident`, which preserves the acceptance criterion's "load-bearing downgrade" meaning.)
