...
| Phase | Status | Notes |
|-------|--------|-------|
| **Phase 0: Frontend/Backend Decoupling** | **Tasks 1–9 Done; Task 10 IN PROGRESS.** Phase 9 BLOCKED. | Adapter-level recording proxy + simulated adapter + 7 captured fixtures + `?midi=simulated` URL-param mode + Vite fixture middleware + Playwright UI specs + drift-detection script. CI workflow removed 2026-05-11. Tests run locally via `make test-ui-roland` / `make test-ui-s3k` / `pnpm test`. **Task 10 status (2026-05-12):** Waves 1 / 2a / 2b / 2c / 3 complete — issues #411–#414 closed 2026-05-12; 137 specs passing via `make test-ui-roland` (up from 31). Wave 4 partial — 13 of 15 capabilities bound (close-out commit `0a87d409` landed Decision 3 fixture-copy seeding + 5 specs). Remaining 2 (D-LIB-14, 21) wait on Wave 5 DnD. Wave 5 (DnD, 4 specs) sequenced next per Decision 4 = Option A. Wave 6 partial — 3 specs landed; remaining 3 (D-XX-02/03/04 DT1 emits) blocked on S-550 front-panel fixture capture; D-XX-01/05/06/07/08 marked `removed` per Decision 1. See [decisions-2026-05-11.md](./decisions-2026-05-11.md) v3 for the gating items. |
...
10. ⏳ **INCOMPLETE.** Capability test suite — every capability in [`ROLAND-S550-EDITOR-CAPABILITIES.md`](../../../../ROLAND-S550-EDITOR-CAPABILITIES.md) must be bound to a passing test before Phase 9 can resume.
   ...
   **Sub-tasks (all required for completion; none deferrable):**
   ...
   - 🟡 **Wave 4 partial** (commits `e14fbe83` + `0a87d409`) — 13 of 15 capabilities bound. Initial cut (`e14fbe83`): 8 capabilities (7 test blocks; D-LIB-11+20 share one) at `test/ui/capabilities/library-flows.spec.ts`. Close-out (`0a87d409`, 2026-05-12, after Decision 3 confirmation): +5 specs at `test/ui/capabilities/library-flows-dialogs.spec.ts` binding D-LIB-12 (Import Tone), D-LIB-13 (Import Patch), D-LIB-17 (Loop Editor), D-LIB-18 (Sample Editor), D-LIB-19 (Sample Chopper) via fixture-copy seeding helpers (`seedOPFSTone`, `seedOPFSPatch`, `seedOPFSSample`) that parse through Zod schemas before writing into OPFS. New fixture authored: `test/e2e/fixtures/samples/basic-sine/sample.{yaml,wav}`. Remaining 2 capabilities (D-LIB-14 ImportSamplesDialog, D-LIB-21 WaveSegmentMap) wait on Wave 5 DnD harness — their only mount path is via drag-and-drop. [#415](https://github.com/audiocontrol-org/audiocontrol/issues/415) stays OPEN until 15/15 land. `make test-ui-roland`: 137 passed.
