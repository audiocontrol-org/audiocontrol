...
| Phase | Status | Notes |
|-------|--------|-------|
| **Phase 0: Frontend/Backend Decoupling** | **Tasks 1–9 Done; Task 10 IN PROGRESS.** Phase 9 BLOCKED. | Adapter-level recording proxy + simulated adapter + 7 captured fixtures + `?midi=simulated` URL-param mode + Vite fixture middleware + Playwright UI specs + drift-detection script. CI workflow removed 2026-05-11. Tests run locally via `make test-ui-roland` / `make test-ui-s3k` / `pnpm test`. **Task 10 status (2026-05-12):** Waves 1 / 2a / 2b / 2c / 3 complete — issues #411–#414 closed 2026-05-12; 132 specs passing via `make test-ui-roland` (up from 31). Wave 4 partial — 8 of 15 capabilities bound (7 test blocks; D-LIB-11+20 share one); remaining 7 unblocked 2026-05-12 by **Decision 3 confirmation** (reuse validated fixtures from `test/e2e/fixtures/` + Zod-schema validation on read). Wave 5 (DnD, 4 specs) sequenced next per Decision 4 = Option A. Wave 6 partial — 3 specs (panic + progress + live-edit guard) landed; remaining 3 (D-XX-02/03/04 DT1 emits) blocked on S-550 front-panel fixture capture; D-XX-01/05/06/07/08 marked `removed` per Decision 1. See [decisions-2026-05-11.md](./decisions-2026-05-11.md) v3 for the gating items. |
...
10. ⏳ **INCOMPLETE.** Capability test suite — every capability in [`ROLAND-S550-EDITOR-CAPABILITIES.md`](../../../../ROLAND-S550-EDITOR-CAPABILITIES.md) must be bound to a passing test before Phase 9 can resume.
   ...
   **Sub-tasks (all required for completion; none deferrable):**
   ...
   - 🟡 **Wave 4 partial** (commit `e14fbe83`) — 8 of 15 capabilities bound (7 test blocks; D-LIB-11+20 share one) at `test/ui/capabilities/library-flows.spec.ts`. **Decision 3 confirmed 2026-05-12** ([decisions-2026-05-11.md](./decisions-2026-05-11.md) v3); fixture-copy seeding approach accepted. Close-out dispatch: ~100-150 line helper + 5 specs (D-LIB-12, 13, 17, 18, 19); remaining 2 specs (D-LIB-14, 21) gated on Wave 5 DnD harness. [#415](https://github.com/audiocontrol-org/audiocontrol/issues/415) stays OPEN until 7/7 land.
