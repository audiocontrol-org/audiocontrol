...
| BUG-006 | 2026-05-27 | PlayPage — Out column doesn't match device VFD | Play page renders Out=1 for every part even when the device's VFD shows different values (operator screenshot 2026-05-27: device shows A/B=1, C/D/E=3, F/G/H=1 for KICK/SNARE/HHC patches; UI shows all 1s). Root cause: `s-series-client.ts:1132-1141` (and identical pattern at `s330-client.ts:1751-1760`) silently catches whatever the `requestDataWithAddress(MULTI_OUTPUTS_ADDRESS, ...)` call throws and substitutes `[1,1,1,1,1,1,1,1]` — comment claims "Output parameters may not exist on all firmware versions" but s330-client.ts's own constants at lines 132-140 explicitly document `0x42-0x49` as canonical MULTI OUTPUT ASSIGN per part, so the justification is fabricated. Same anti-pattern in the outer `requestFunctionParameters` catch (lines 1159-1167 / 1778-1786 — returns whole-config defaults silently) and in `setMultiOutput`'s read-modify-write (lines 1231-1239 / 1850-1858 — worse failure mode: if the read fails, a write of a single output silently resets ALL 8 to 1). Step 1: strip the 6 silent catches across both clients so the actual underlying error surfaces via the existing PlayPage `setError` → page-level banner. Step 2 (separate commit, after the operator reproduces with stripped catches): fix the underlying read failure based on the real error message. | Step 1 in progress | — |
