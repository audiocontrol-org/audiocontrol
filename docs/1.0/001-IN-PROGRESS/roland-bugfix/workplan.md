...
| 2026-05-22 | `62705162bc69` (24L) + `96898287ce31` (21L) + `4b372a9e9e18` (8L) | EnvelopeDisplay.tsx ↔ EnvelopeEditor.tsx (SVG envelope curve render: grid lines, sustain marker, path build, point circles) | keep-with-reason | EnvelopeDisplay.tsx is marked `@deprecated` by its own header — Phase 9 Task 4's TonesPage redesign introduced `AcEnvelope` (editor-core) as the canonical envelope surface (see `ToneEnvelopeEditor`); the file is kept solely for an audit-and-delete dispatch. Source-tree grep confirms zero consumers (only the re-export from `ui/index.ts` keeps the symbol public). Extracting a shared helper from a file scheduled for deletion would be churn. Disposition: kept until the audit-and-delete dispatch lands. **Follow-up: ROLAND-BUGFIX-DEL-001** — delete `modules/roland-sxx0-editor/src/components/ui/EnvelopeDisplay.tsx` + remove its export from `ui/index.ts`. The deletion was attempted inline as part of this Phase 2 walk and blocked by the auto-mode permission classifier (irreversible source-file deletion needs explicit operator authorization). The follow-up requires that authorization. |
...
| 2026-05-22 | `c7df8647daf8` (18L) + `325d993bda15` (12L) | CreateDirectoryDialog.tsx ↔ RenameDirectoryDialog.tsx (Dialog.Portal + Overlay + Content shell, "Location" display, "Folder Name" / "New Name" input + validation, Cancel + submit row) | keep-with-reason | Both dialogs are dead code — defined in `src/components/library/` but never instantiated by the production LibraryPage. Folder CRUD is owned by editor-core's LibraryBrowser + its own CreateFolderDialog. Documented at `modules/roland-sxx0-editor/test/ui/in-context/library-dialogs.in-context.spec.ts:326-335`: *"orphan components … dead-code status tracked separately and should be resolved by deletion, not by accreting dev-only mount paths."* The companion `useDirectoryOperations.ts` hook is also unused (grep confirms zero consumers). Extracting a shared helper from files scheduled for deletion would be churn. **Follow-up: ROLAND-BUGFIX-DEL-002** — delete `CreateDirectoryDialog.tsx`, `RenameDirectoryDialog.tsx`, `useDirectoryOperations.ts`. Operator approval required for the source-file deletions per the auto-mode permission classifier (same precedent as ROLAND-BUGFIX-DEL-001). |
...
Refactor commits landed during Phase 2 (10):
...
- **ROLAND-BUGFIX-DEL-001** — delete `EnvelopeDisplay.tsx` + `EnvelopeEditor.tsx` (needs operator approval per auto-mode permission classifier)
- **ROLAND-BUGFIX-DEL-002** — delete `CreateDirectoryDialog.tsx` + `RenameDirectoryDialog.tsx` + `useDirectoryOperations.ts` (same)
- **ROLAND-BUGFIX-DEL-003** — delete `modules/roland-sxx0-editor/scripts/watchdog.ts` (same)
