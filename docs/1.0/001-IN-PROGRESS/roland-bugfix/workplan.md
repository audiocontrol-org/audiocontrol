...
| BUG-006 | 2026-05-27 | PlayPage — Out column doesn't match device VFD | Play page renders Out=1 for every part even when the device's VFD shows different values (operator screenshot 2026-05-27: device shows A/B=1, C/D/E=3, F/G/H=1 for KICK/SNARE/HHC patches; UI shows all 1s). Step 1 (commit `0042fc02`): stripped 6 silent catches in both clients. Step 2 (2026-05-27): empirical probe confirmed MULTI OUTPUT ASSIGN does NOT exist in function parameters (addresses 0x42-0x56 return garbage). The correct address is in each part's assigned **patch** block at `[0x03, 0x66]` (OUTPUT ASSIGN) and `[0x03, 0x5a]` (OUTPUT LEVEL). Fixed `requestFunctionParameters()`, `setMultiOutput()`, and `setMultiLevel()` in both `s-series-client.ts` and `s330-client.ts` to read from / write to per-patch addresses. Output convention: device stores 0-indexed (0=OUTPUT 1); PlayPage uses 1-indexed (1=OUTPUT 1); +1/-1 conversion applied at the client boundary. Probe verification: after fix, `requestFunctionParameters()` returns Out=[1,2,3,3,3,1,1,1] matching device state. `make test-ui-roland`: 4 passed, 2 skipped. Protocol doc updated: `s550-sysex-protocol.md` now documents the correct per-patch output/level addresses. Dead `S330_FUNCTION_ADDRESSES.MULTI_OUTPUT_*` / `MULTI_LEVEL_*` constants removed (commit `36fa90b0`) — zero source consumers confirmed before deletion per agent-discipline.md. | Awaiting operator retest | `0042fc02` (step 1) → `4558d00a` (step 2) → `36fa90b0` (cleanup) |
...
- **Two gaps surfaced via dogfooding:**
  1. **Exception silencing masks cross-editor holdouts in the matrix.** The akai-s3k-editor cell for `slide-drawer-library-dialogs` rendered as `✓ 9/9` despite zero actual adopters — exceptions were subtracted before the matrix renders. Same root cause as #453 (no `tracked_holdouts:` distinction from permanent exceptions). **Closed:** #453 landed in PR #454 (commit `caa132d9`) and the manifest now classifies the 9 Akai deferrals as `tracked_holdouts`; the matrix gained the `⏳` cell-state and the visibility gap is closed.
...
**Scope statement vs branch identity:** This is enhancement work, not a bug-fix. The branch's "Out of Scope" rule (below) says new features get their own branch; the operator's 2026-06-01 ask explicitly overrides that for this enhancement. The work co-locates with Phase 1 (bug-fix) and Phase 2–7 (scope-discovery validation) under the same per-commit discipline; the cross-editor promotion validates Phase 5/6 dogfooding patterns (new adopter-manifest + anti-pattern entries).

**Why promote to editor-core (not copy into Roland):** DRY per project CLAUDE.md, and the same shape Phase 6 already used for `slide-drawer-library-dialogs`. The Akai version uses a generic LPF biquad approximation parameterized only by `FREQ_MAX = 99` and `Q_MAX = 15`; making those props is one rename. Once promoted, both editors share the rendering + drag math + CSS, and the adopter-manifest + anti-pattern gates prevent future re-inlining.
...
- **T8.6 — Test-first protecting assertions.** Per Phase 3's refactor protocol (T8.1+T8.2 IS a cross-module extraction):
...
Anything surfaced becomes either an additional T8.x sub-task (migrate now) or a `ROLAND-BUGFIX-FILTER-EXT-NNN` follow-up issue (if scope is genuinely separate) with explicit operator acceptance per `agent-discipline.md`.
