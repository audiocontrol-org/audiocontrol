...
| Phase | Status | Notes |
|-------|--------|-------|
| Phase 0: Bug-fix — tab active-indicator tracks selected tab (`AcRadioTabs`) | Done; pending operator review | `AcRadioTabs` made controlled (`useState` drives `checked` + `aria-selected`), killing the uncontrolled-radio stale-paint race. `D-TAB-INDICATOR-01a/b` (tones + patches) lands with the fix, fails against `6fe066a6`. Screenshots confirm FILTER (Tones) + MAPPING (Patches) highlight correctly. See [workplan.md](./workplan.md) §Phase 0 |
