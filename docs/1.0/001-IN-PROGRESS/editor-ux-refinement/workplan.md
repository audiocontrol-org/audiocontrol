...
- **P2.3 — Deterministic capture step.** Playwright navigates `route?midi=simulated&scenario=<capture>`, awaits an explicit page **ready hook + `document.fonts.ready`** (no `sleep`/defensive delays, per `feedback_no_delays`), and shoots at the pinned **1280×800 logical @2x (2560×1600)**, full app-shell, no browser chrome. **Proven complete when:** two consecutive captures of the same scene are perceptually identical (a determinism test asserts this); no arbitrary timeouts appear in the capture path.
  - **DECIDED approach (operator, 2026-06-01): reuse the pattern in e2e-infra** — do NOT duplicate the existing `test/rendering/` smokes (which already device-free-capture Roland pages incl. `tones-filter` for s330+s550 via `defineHarnessConfig` + `page.goto(... ?midi=simulated&scenario=...)` + `waitForLoadState('networkidle')` + `page.screenshot({fullPage:true})`). **Concrete build plan for the next session:**
    1. Refactor `modules/e2e-infra/scripts/dev-server-lib.sh`: extract `run_with_dev_server <cmd...>` (start vite on OS port → export `E2E_PORT` → run cmd → `ac_kill_tree` teardown); `run_playwright_harness` becomes a thin caller. **Re-verify `make test-ui-roland` + `make test-ui-s3k` after the refactor** (the launcher is load-bearing).
    2. `modules/e2e-infra/src/promo/capture.ts` — Playwright API (`chromium` from `@playwright/test`), reads `E2E_PORT` + `PROMO_EDITOR`, filters `PROMO_SCENES` to that editor, per scene: `newContext({viewport, deviceScaleFactor})` → `goto(baseURL+route)` → `waitForLoadState('networkidle')` + `page.evaluate(() => document.fonts.ready)` → `screenshot({fullPage:true})` → `out/promo/<id>.png` (resolved from the spec dir, `../../../../out/promo`).
    3. `make promo-shots`: for roland then akai, `cd` editor + source the lib + `PROMO_EDITOR=<x> run_with_dev_server npx tsx ../e2e-infra/src/promo/capture.ts`.
    4. Determinism check: capture a scene twice, assert perceptual equality.
