...
- **T8.8 — Visual verification (above-the-fold).** ⚠ **Partially done — OPERATOR DECISION NEEDED.** Captured the Roland FILTER tab device-free (curve renders + drags correctly — `.ac-curve-display` present). Measured `getBoundingClientRect()`: the curve bottom edge sits at **y≈1010px**. So both envelope + curve are above-the-fold **only at viewports ≥ ~1010px tall**; at the short test viewports (720–800px) the curve falls below the fold (envelope is above, curve ~54px under at 800px). The reorder DID move both viz above the slider grid (the workplan's other goal), but "both above-the-fold" depends on the target viewport.
  - **Decision needed:** (a) accept current stacked order (fine on ≥1010px-tall screens), (b) make the two viz **side-by-side** at the top (needs both narrower than the ~720px detail pane), (c) **curve-first** then envelope (curve is shorter — no segment table — so curve clears the fold even at 720px), or (d) trim the envelope's segment-table height. T8.3 specified the stacked envelope→curve order; this is the layout-refinement call the visual check surfaced.
