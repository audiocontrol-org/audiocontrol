...
### Phase 6 regime-holdout artifacts

Four additional repo-level artifacts ship with Phase 6 (Regime-Holdout Discovery). They detect regime drift that does not surface as a token-level clone — code that "missed the migration" or "doesn't yet duplicate enough to trip the clone detector." See [`README.md`](README.md) §"Regime-holdout discovery" for the conceptual model; this section is the on-disk contract for each artifact.

#### `docs/scope-discovery/anti-patterns.yaml`

Registry of legacy structural shapes that should be replaced by extracted primitives. Each refactor commit that EXTRACTS a primitive appends an entry naming the shape it replaces. Committed.

Schema header (canonical at [`anti-patterns.yaml`](anti-patterns.yaml) docblock):

```yaml
anti_patterns:
  - id: <kebab-case id; unique>
    added_in: <commit-sha>
    primitive: <component or hook name>
    from: <import path>
    shape_regex: <regex string OR list of regex strings>
    min_distance: <integer; max line gap when shape_regex is a list; default 50>
    message: <multi-line replacement instruction>
```

- **Validator:** [`../../tools/scope-discovery/check-anti-patterns.ts`](../../tools/scope-discovery/check-anti-patterns.ts)
- **Make targets:** `make check-anti-patterns` (read-only)
- **Pre-commit gate:** **blocks** on TS/TSX changes that match any registered shape

#### `docs/scope-discovery/adopter-manifests.yaml`

Per-primitive declaration of expected adopter files (globs) plus per-path exceptions and tracked-holdouts. Each refactor commit that PROMOTES a primitive to a shared location appends an entry. Committed.

Schema header (canonical at [`adopter-manifests.yaml`](adopter-manifests.yaml) docblock):

```yaml
adopter_manifests:
  - id: <kebab-case id; unique>
    introduced_in: <commit-sha>
    ...
    tracked_holdouts:                  # AUDIT-06; gate-passing deferrals
      - path: <repo-relative path>
        issue: <URL or '#'-prefixed ref>
        reason: <multi-line explanation>
    message: <multi-line holdout instruction>
```

`tracked_holdouts:` (AUDIT-20260522-06) distinguishes deferred-but-known holdouts from permanent opt-outs (`exceptions:`). Tracked holdouts do NOT block the gate (exit 0 when only tracked-holdouts remain) but ARE surfaced in their own report section + render as `⏳ A/E (T tracked)` in the editor-symmetry matrix. Each entry MUST have `path` + `issue` (URL/ref) + `reason`; a path cannot appear in both `exceptions:` and `tracked_holdouts:`.

- **Validator:** [`../../tools/scope-discovery/check-adopters.ts`](../../tools/scope-discovery/check-adopters.ts)
- **Make targets:** `make check-adopters` (read-only)
- **Pre-commit gate:** **blocks** on TS/TSX changes whose paths match any manifest glob but do not import the canonical `from`

#### `docs/scope-discovery/editor-symmetry.md`

Operator-readable fleet matrix. Rows are adopter-manifest entries; columns are editor modules under `modules/*-editor/`; cells show adoption status (`✓ N/N` / `⚠ A/E (H holdout(s))` / `✗` / `⏳ A/E (T tracked)` / `—`). Regenerated by `make check-editor-symmetry-write`; the artifact is committed. Empty when `adopter-manifests.yaml` is empty. The `⏳` glyph (AUDIT-20260522-06) renders for cells whose only non-adopters are operator-deferred via `tracked_holdouts:` in the manifest; gate-passing, NOT masked as `✓`.

- **Validator:** [`../../tools/scope-discovery/check-editor-symmetry.ts`](../../tools/scope-discovery/check-editor-symmetry.ts)
- **Make targets:** `make check-editor-symmetry` (read-only; prints to stdout) and `make check-editor-symmetry-write` (rewrites the artifact)
- **Pre-commit gate:** runs read-only on TS/TSX changes; **non-blocking** beyond the underlying adopter-manifest gate (the symmetry matrix is a presentation of the same data)

#### `docs/scope-discovery/deprecation-queue.md`

Operator-readable list of files carrying a file-level deprecation marker (top-of-file JSDoc `@deprecated` or `// DEPRECATED:` within the first 20 lines). Each row counts external importers; the file is split into "blocked (importers > 0)" and "safe to delete (importers === 0)". Regenerated by `make check-deprecations-write`; the artifact is committed. Symbol-level deprecation (a `@deprecated` tag on one export in an otherwise live file) is OUT OF SCOPE in v1; documented at the scanner's top docblock.

- **Validator:** [`../../tools/scope-discovery/check-deprecations.ts`](../../tools/scope-discovery/check-deprecations.ts)
- **Make targets:** `make check-deprecations` (read-only) and `make check-deprecations-write` (rewrites the artifact)
- **Pre-commit gate:** **informational, not blocking** — the deprecation lifecycle is operator-driven; exit code is 0 regardless of importer count (only exit 2 on infra error)

The five-agent `/scope-inventory` fleet's fifth agent ([`../../tools/scope-discovery/discovery-agents/regime-holdout-detector.ts`](../../tools/scope-discovery/discovery-agents/regime-holdout-detector.ts), Phase 6 T6.5) consumes the first four scanners in-process and emits a `RegimeHoldoutFindings` payload that the synthesis pass writes into the manifest's top-level `regime_holdouts:` section. See [`README.md`](README.md) §"Regime-holdout discovery" for the composition model.
...
- Regime-holdout discovery (conceptual model) — [`README.md`](README.md) §"Regime-holdout discovery"
- Regime-holdout-detector agent (Phase 6 T6.5) — [`../../tools/scope-discovery/discovery-agents/regime-holdout-detector.ts`](../../tools/scope-discovery/discovery-agents/regime-holdout-detector.ts)
