...
Per-primitive declaration of expected adopter files (globs) plus per-path exceptions. Each refactor commit that PROMOTES a primitive to a shared location appends an entry. Committed.
...
Operator-readable fleet matrix. Rows are adopter-manifest entries; columns are editor modules under `modules/*-editor/`; cells show adoption status (`✓ N/N` / `⚠ A/E (H holdout(s))` / `✗` / `—`). Regenerated by `make check-editor-symmetry-write`; the artifact is committed. Empty when `adopter-manifests.yaml` is empty.
