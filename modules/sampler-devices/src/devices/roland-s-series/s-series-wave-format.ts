/**
 * S-Series wave data format encoding/decoding.
 *
 * The S-series uses 12-bit linear PCM samples, transmitted over MIDI as
...
import type { SSeriesWaveSampleRate } from './s-series-types.js';
...
/**
 * S-series wave data ready for transmission.
 */
export interface SSeriesWaveData {
    /** Encoded wave bytes (2 bytes per 12-bit sample, 7-bit MIDI safe) */
    data: Uint8Array;
    /** Sample rate */
    sampleRate: SSeriesWaveSampleRate;
    /** Number of samples */
    sampleCount: number;
}
...
/**
 * Convert WAV data to S-series wave format.
 *
 * @param wav - Parsed WAV data
 * @param targetSampleRate - Target S-series sample rate (15000 or 30000)
 * @returns S-series encoded wave data
 */
export function wavToSeries(wav: WavData, targetSampleRate: SSeriesWaveSampleRate): SSeriesWaveData {
    // Resample if needed
...
/**
 * Convert S-series wave data to 16-bit PCM samples.
 *
 * @param seriesData - S-series encoded wave bytes
 * @param _sampleRate - Sample rate (15000 or 30000) - unused but kept for API consistency
 * @returns 16-bit PCM samples
 */
export function seriesToWav(seriesData: Uint8Array, _sampleRate: SSeriesWaveSampleRate): Int16Array {
    const sampleCount = Math.floor(seriesData.length / 2);
    const samples = new Int16Array(sampleCount);

    for (let i = 0; i < sampleCount; i++) {
        const byte0 = seriesData[i * 2];
        const byte1 = seriesData[i * 2 + 1];
 ...
 *
 * Each S-series segment holds 12,000 samples.
 */
...
/**
 * Prepared S-series sample data ready for device upload.
 */
export interface PreparedSSeriesSample {
...
/**
 * Convert raw WAV file bytes to S-series format.
 *
 * This is the CANONICAL function for WAV -> S-series conversion.
 ...
 */
export function prepareWavForSSeries(
    wavBytes: ArrayBuffer,
    targetSampleRate: SSeriesWaveSampleRate,
    ...
    const seriesData = wavToSeries(wavData, targetSampleRate);
    const segmentLength = calculateSegmentsNeeded(seriesData.sampleCount);
    ...
    if (options?.enableDiagnostics) {
        ...
        const actualRatio = wavData.samples.length / seriesData.sampleCount;
        const expectedRatio = wavData.sampleRate / targetSampleRate;

        console.log(`[prepareWavForSSeries] ========================================`);
        console.log(`[prepareWavForSSeries] SOURCE WAV:`);
        console.log(`[prepareWavForSSeries]   Sample rate: ${wavData.sampleRate} Hz`);
        console.log(`[prepareWavForSSeries]   Bit depth: ${wavData.bitsPerSample}-bit`);
        console.log(`[prepareWavForSSeries]   Channels: ${wavData.channels}`);
        console.log(`[prepareWavForSSeries]   Sample count: ${wavData.samples.length}`);
        console.log(`[prepareWavForSSeries]   Duration: ${source.duration.toFixed(3)}s`);
        console.log(`[prepareWavForSSeries] TARGET:`);
        console.log(`[prepareWavForSSeries]   Sample rate: ${targetSampleRate} Hz`);
        console.log(`[prepareWavForSSeries]   Expected samples: ${expectedOutputSamples}`);
        console.log(`[prepareWavForSSeries] RESULT:`);
        console.log(`[prepareWavForSSeries]   Output samples: ${seriesData.sampleCount}`);
        console.log(`[prepareWavForSSeries]   Output bytes: ${seriesData.data.length}`);
        console.log(`[prepareWavForSSeries]   Segments: ${segmentLength}`);
        console.log(`[prepareWavForSSeries]   Output duration: ${(seriesData.sampleCount / targetSampleRate).toFixed(3)}s`);

        if (Math.abs(actualRatio - expectedRatio) > 0.01) {
            console.error(`[prepareWavForSSeries] WARNING: RESAMPLING MISMATCH!`);
            console.error(`[prepareWavForSSeries]   Actual ratio: ${actualRatio.toFixed(4)}`);
            console.error(`[prepareWavForSSeries]   Expected ratio: ${expectedRatio.toFixed(4)}`);
        } else {
            console.log(`[prepareWavForSSeries] Resampling verified: ${actualRatio.toFixed(4)}x`);
        }
    }

    return {
        data: seriesData.data,
        sampleCount: seriesData.sampleCount,
        segmentLength,
