 ...
 * on-disk prelude and applying the canonical encoder. Supports two modes:
 * `regen` (rewrite an existing fixture's writeback in place) and
 * `--init --from-base` (create a new fixture by copying a base prelude
 * and synthesizing the writeback against a new scenario mutation).
 *
 * See `./synthesize-tone-fixture.md` for full CLI usage, mode semantics,
 * determinism contract, and the rules for when synthesis is and is not
 * an appropriate substitute for a fresh hardware recording.
 */
...
function main(): number {
    ...
    if (opts.init) {
        ...
        try {
            writeFileSync(fixturePath, serializeFixture(created), { encoding: 'utf8', flag: 'wx' });
        } catch (err) {
