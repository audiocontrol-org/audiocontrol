...
import { sliceBySilence, trimSilence } from '@/sample-chopper/silence-detector.js';
...
describe('sliceBySilence', () => {
  it('should detect silence gaps and split into regions', () => {
    // Create: 100ms sound, 100ms silence, 100ms sound
  ...
  it('should ignore short silence gaps', () => {
    // Create: 100ms sound, 20ms silence (too short), 100ms sound
    const samples = createTestSignal(10000, [
      { durationMs: 100, amplitude: 0.5 },
    ...
    const result = sliceBySilence(samples, 10000, {
      method: 'silence',
      thresholdDb: -40,
      minSilenceMs: 50, // 20ms gap should be ignored
    });

    // Should merge into one region since gap is too short
    expect(result.slices).toHaveLength(1);
    expect(result.slices[0]?.durationMs).toBeCloseTo(220, -1);
  });

  it('should filter out short samples', () => {
    // Create: 5ms sound, 100ms silence, 100ms sound
    // Note: The 10ms analysis window means very short sounds at the start
    // may be detected differently depending on alignment
    const samples = createTestSignal(10000, [
      { durationMs: 5, amplitude: 0.5 }, // Too short - should be filtered
      { durationMs: 100, amplitude: 0 },
      { durationMs: 100, amplitude: 0.5 },
    ]);
    ...
    const result = sliceBySilence(samples, 10000, {
      ...
      thresholdDb: -40,
      minSilenceMs: 50,
      minSampleMs: 50, // Use higher threshold to ensure 5ms is filtered
    });

    // Only the 100ms sound region should be returned
    expect(result.slices).toHaveLength(1);
    // Allow for window alignment variance
  ...
  it('should handle completely silent signal', () => {
    const samples = new Int16Array(5000); // All zeros
  ...
  it('should assign sequential indices', () => {
    // Create: 3 sound regions with silence between
    const samples = createTestSignal(10000, [
      { durationMs: 100, amplitude: 0.5 },
      { durationMs: 100, amplitude: 0 },
      { durationMs: 100, amplitude: 0.5 },
...
describe('trimSilence', () => {
  it('should find start and end of non-silent content', () => {
    // Create: 100ms silence, 200ms sound, 100ms silence
    const samples = createTestSignal(10000, [
      { durationMs: 100, amplitude: 0 },
    ...
    // Start should be around sample 1000 (100ms at 10kHz)
    expect(start).toBeCloseTo(1000, -2);

    // End should be around sample 3000 (300ms at 10kHz)
