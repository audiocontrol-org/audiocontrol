 ...
 *
 * Uses design token CSS classes (ac-*) instead of device-specific styling.
 */
...
export function WaveformEditor({
  ...
  if (!samples || samples.length === 0) {
    return (
      <div
        className={cn(
          'bg-ac-bg rounded border border-ac-accent/30 flex items-center justify-center',
          className
        )}
        style={{ height }}
      >
        <span className="text-ac-muted text-sm">No audio loaded</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Zoom controls */}
      {showZoomControls && (
        <div className="absolute top-1 right-1 z-10 flex items-center gap-1 bg-ac-bg/80 rounded px-1">
          <button
          ...
          <button
            ...
            className="px-1 text-xs text-ac-muted hover:text-ac-text transition-colors"
            title="Reset zoom (0)"
          >
      ...
      >
        <canvas
          ...
          className={cn('rounded border border-ac-accent/30 outline-none')}
          style={{ cursor: getCursor() }}
        />
      </div>

      {/* Duration label */}
      <div className="absolute bottom-5 right-2 text-xs text-ac-muted pointer-events-none">
      ...
      {editable && !dragState && sliceMarkers.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="text-ac-muted text-sm bg-ac-bg/80 px-2 py-1 rounded">
      ...
      {zoom > 1 && (
        <div className="absolute bottom-5 left-2 text-xs text-ac-muted pointer-events-none">
