...
describe('ToneZoneEditor utilities', () => {
  describe('arrayToZones', () => {
    ...
    it('should parse multiple contiguous zones', () => {
      // Layer 1: -1 means OFF
      const data = new Array(TOTAL_KEYS).fill(-1);
      // Keys 12-23 (indices 0-11): Tone 0
      for (let i = 0; i < 12; i++) data[i] = 0;
      // Keys 24-35 (indices 12-23): Tone 1
      for (let i = 12; i < 24; i++) data[i] = 1;
      // Keys 36-47 (indices 24-35): Tone 2
      for (let i = 24; i < 36; i++) data[i] = 2;

      const zones = arrayToZones(data, 1);
      expect(zones).toHaveLength(3);
      expect(zones[0]).toEqual({ startKey: MIN_KEY, endKey: 23, tone: 0 });
      expect(zones[1]).toEqual({ startKey: 24, endKey: 35, tone: 1 });
      expect(zones[2]).toEqual({ startKey: 36, endKey: 47, tone: 2 });
    });
    ...
    it('should skip OFF entries between zones', () => {
      ...
      // Keys 12-23 (indices 0-11): Tone 0
      for (let i = 0; i < 12; i++) data[i] = 0;
      // Keys 36-47 (indices 24-35): Tone 2 (gap at indices 12-23)
      for (let i = 24; i < 36; i++) data[i] = 2;

      const zones = arrayToZones(data, 1);
      expect(zones).toHaveLength(2);
      expect(zones[0]).toEqual({ startKey: MIN_KEY, endKey: 23, tone: 0 });
      expect(zones[1]).toEqual({ startKey: 36, endKey: 47, tone: 2 });
    });

    it('should handle Layer 2 with 0 as OFF value', () => {
      const data = new Array(TOTAL_KEYS).fill(0); // Layer 2: 0 means OFF
      // Keys 12-23 (indices 0-11): Tone 5
      for (let i = 0; i < 12; i++) data[i] = 5;

      const zones = arrayToZones(data, 2);
      expect(zones).toHaveLength(1);
      expect(zones[0]).toEqual({ startKey: MIN_KEY, endKey: 23, tone: 5 });
    });
    ...
    it('should handle non-contiguous same-tone zones as separate zones', () => {
      ...
      // Keys 12-23 (indices 0-11): Tone 5
      for (let i = 0; i < 12; i++) data[i] = 5;
      // Keys 36-47 (indices 24-35): Tone 5 (same tone, but not contiguous)
      for (let i = 24; i < 36; i++) data[i] = 5;

      const zones = arrayToZones(data, 1);
      expect(zones).toHaveLength(2);
      expect(zones[0]).toEqual({ startKey: MIN_KEY, endKey: 23, tone: 5 });
      expect(zones[1]).toEqual({ startKey: 36, endKey: 47, tone: 5 });
    });

    it('should handle single-key zones', () => {
      const data = new Array(TOTAL_KEYS).fill(-1);
      data[0] = 3; // Just the first key (MIDI 12)
  ...
  describe('zonesToArray', () => {
    ...
    it('should correctly place multiple zones', () => {
      const zones: ToneZone[] = [
        { startKey: MIN_KEY, endKey: 23, tone: 0 },  // indices 0-11
        { startKey: 24, endKey: 35, tone: 1 },       // indices 12-23
        { startKey: 36, endKey: 47, tone: 2 },       // indices 24-35
      ];
      const data = zonesToArray(zones, 1);
    ...
    it('should handle overlapping zones (last zone wins)', () => {
      const zones: ToneZone[] = [
        { startKey: MIN_KEY, endKey: 35, tone: 0 },  // indices 0-23
        { startKey: 24, endKey: 47, tone: 1 },       // indices 12-35, overlaps
      ];
      const data = zonesToArray(zones, 1);

      // Keys 12-23 (indices 0-11) should be tone 0
      for (let i = 0; i < 12; i++) {
        expect(data[i]).toBe(0);
      }
      // Keys 24-47 (indices 12-35) should be tone 1 (overwritten by second zone)
      for (let i = 12; i < 36; i++) {
        expect(data[i]).toBe(1);
      }
    });
    ...
    it('should handle Layer 2 with 0 as OFF value', () => {
      const zones: ToneZone[] = [{ startKey: MIN_KEY, endKey: 23, tone: 5 }];
      const data = zonesToArray(zones, 2);
    ...
    it('should clamp keys to valid range', () => {
      const zones: ToneZone[] = [
        { startKey: 5, endKey: 21, tone: 5 },    // startKey below MIN_KEY
        { startKey: 113, endKey: 130, tone: 6 }, // endKey above MAX_KEY
      ];
      const data = zonesToArray(zones, 1);

      // Only valid keys should be filled
      // Keys 12-21 (indices 0-9) should be tone 5
      for (let i = 0; i <= 9; i++) {
        expect(data[i]).toBe(5);
      }
      // Keys 113-120 (indices 101-108) should be tone 6
