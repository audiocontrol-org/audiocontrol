...
```bash
npm install -g @audiocontrol/audiotools
```
...
If you previously used `@audiocontrol/sampler-backup` or `@audiocontrol/sampler-export` individually:
...
**Solution:**
1. Ensure installed globally: `npm install -g @audiocontrol/audiotools`
...
```typescript
import { backupCommand } from '@audiocontrol/audiotools/commands/backup';
import { exportCommand } from '@audiocontrol/audiotools/commands/export';
  ...
  backup:
    ...
    steps:
      - name: Install audiotools
        run: npm install -g @audiocontrol/audiotools
...
- **Configuration library**: `@audiocontrol/audiotools-config` (dependency)
- **Backup tool**: `@audiocontrol/sampler-backup` (dependency)
- **Export tool**: `@audiocontrol/sampler-export` (dependency)

## Architecture

```
@audiocontrol/audiotools (this package)
├── commands/
│   ├── backup.ts    → delegates to @audiocontrol/sampler-backup
│   ├── export.ts    → delegates to @audiocontrol/sampler-export
│   └── config.ts    → delegates to @audiocontrol/audiotools-config
├── index.ts         → CLI entry point
└── dependencies:
    ├── @audiocontrol/audiotools-config  → Shared configuration
    ├── @audiocontrol/sampler-backup     → Backup functionality
    └── @audiocontrol/sampler-export     → Export functionality
```

## Related Packages

- **[@audiocontrol/audiotools-config](../audiotools-config/README.md)** - Configuration library
- **[@audiocontrol/sampler-backup](../sampler-backup/README.md)** - Backup functionality
- **[@audiocontrol/sampler-export](../sampler-export/README.md)** - Export functionality
