#!/usr/bin/env bash
#
# Run device-library e2e tests using HTTP MIDI transport.
#
# This script:
...
#
# Usage:
#   ./scripts/run-device-library-e2e.sh [playwright args...]
...
echo "=== Device-Library E2E Test Runner ==="
echo ""
...
echo "Step 5: Running device-library e2e tests..."
echo ""
...
if [ "$PLAYWRIGHT_EXIT" -eq 137 ]; then
  ...
  echo "Test runner was killed by watchdog (no activity for 5s)."
  exit 1
fi
