/**
 * Generic tree icons for library UI components.
 * Device-agnostic SVG icons using ac- class prefix.
 */
...
export function FolderIcon({ isOpen }: { isOpen: boolean }): JSX.Element {
  return (
    <svg
      className={`ac-tree-icon ${isOpen ? 'ac-tree-icon--active' : ''}`}
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
    >
...
export function CloneIcon(): JSX.Element {
  return (
    <svg className="ac-tree-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path
        strokeLinecap="round"
