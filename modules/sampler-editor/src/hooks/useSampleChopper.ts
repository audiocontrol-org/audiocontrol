 ...
 * manual editing, strip silence, zoom, kit configuration, and slice manipulation.
 */
...
import type { SliceMarker, SliceChange } from '@/components/library/WaveformEditor';
...
export function useSampleChopper({
  samples, sampleRate, open, editMode, initialSlices, initialKitConfig,
  ...
  const [kitSampleRate, setKitSampleRate] = useState<15000 | 30000>(initialKitConfig?.sampleRate ?? 15000);
  ...
  const [kitVelocitySensitivity, setKitVelocitySensitivity] = useState(initialKitConfig?.velocitySensitivity ?? 2);
  ...
  return {
    ...
    handleApplyStripSilence, handleCancelStripSilence,
    ...
    kitName, setKitName, kitLabels, setKitLabels,
    kitSampleRate, setKitSampleRate, kitBaseNote, setKitBaseNote,
    kitTranspose, setKitTranspose, kitVelocitySensitivity, setKitVelocitySensitivity,
