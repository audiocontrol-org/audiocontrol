 ...
 * migrated in commit bba5b13b. Closes AUDIT-20260524-05.
 *
 ...
 *
 * The harness routes (TestProgramsPage / TestSamplesPage /
 * TestLibraryPage at /akai/s3000xl/editor/test/{programs,samples,library})
 * render the SAME shell scaffold the production pages render, but
 * with factory data + stub callbacks so the spec doesn't need a
 * device / store hydration / MIDI server.
 *
 * The fourth route — `/test/keygroups` — currently points to the
 * pre-existing TestKeygroupsPage which renders an inline-styled
 * zone-overview interaction harness, NOT the production shell. The
 * `KEYGROUPS_SHELL_HARNESS_AVAILABLE` toggle below is the seam where
 * a future shell-compliant keygroups harness can opt in; today it
 * stays false so this spec doesn't false-fail on a known divergence.
 * Tracked in DEVELOPMENT-NOTES (see the AUDIT-20260524-05 closure
 * entry) — the keygroups production page IS shell-compliant (covered
 * by the migration commit and by the cross-page CSS gates); the only
 * gap is the harness, not the production code under audit.
 *
...
const KEYGROUPS_SHELL_HARNESS_AVAILABLE = false;
...
if (KEYGROUPS_SHELL_HARNESS_AVAILABLE) {
  // Reserved seam — see file header. A keygroups shell-harness page
  // would be added here once it exists.
}
