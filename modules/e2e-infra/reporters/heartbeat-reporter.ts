...
/**
 * Regex matching MIDI client activity from any supported device.
 * Used to detect ongoing MIDI communication and keep the heartbeat alive.
 */
const MIDI_ACTIVITY_PATTERN = /S330Client|S550Client|S3000XL|AkaiClient|BROWSER:/;
...
class HeartbeatReporter implements Reporter {
  ...
  constructor() {
    // Use env var if set (from run-http-midi-e2e.sh), otherwise use PID
  ...
  onStdOut(chunk: string | Buffer, _test?: TestCase, _result?: TestResult): void {
    ...
    if (MIDI_ACTIVITY_PATTERN.test(text)) {
