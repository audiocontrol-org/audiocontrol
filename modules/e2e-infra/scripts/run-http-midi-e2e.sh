#!/usr/bin/env bash
#
# Shared HTTP MIDI E2E test runner.
#
# This script is called FROM an editor module directory. It:
#   1. Validates device is connected (via $E2E_VALIDATE_CMD)
...
#   - Device connected via MIDI
#
# Required environment variables:
#   E2E_VALIDATE_CMD - Command to validate device connectivity
#                      (e.g., "tsx /path/to/validate-akai-s3000xl.ts")
#
# Optional environment variables:
#   E2E_PLAYWRIGHT_CONFIG - Playwright config file (default: playwright.http-midi.config.ts)
#   MIDI_SERVER_BIN - Path to midi-server binary
#
# Usage:
#   E2E_VALIDATE_CMD="tsx scripts/validate-device.ts" ./run-http-midi-e2e.sh [playwright args...]
...
# The caller's working directory is the editor module
PROJECT_DIR="$(pwd)"
...
if [ -z "${E2E_VALIDATE_CMD:-}" ]; then
  echo ""
  echo "ERROR: E2E_VALIDATE_CMD is not set."
  echo ""
  echo "Set it to the command that validates device connectivity, e.g.:"
  echo "  export E2E_VALIDATE_CMD=\"tsx /path/to/validate-akai-s3000xl.ts\""
  echo ""
  exit 1
fi

DEVICE_INFO=$(eval "$E2E_VALIDATE_CMD" 2>&1) || {
  echo ""
...
PLAYWRIGHT_CONFIG="${E2E_PLAYWRIGHT_CONFIG:-playwright.http-midi.config.ts}"
