 ...
 *
 * Implements the D110MidiIO interface using the Web MIDI API,
 * enabling direct browser-to-hardware communication.
 *
 * Handles chunked SysEx messages that may be split across multiple events.
 */

import type { D110MidiIO, SysExCallback, MidiPortInfo, WebMidiAccess } from '@/core/midi/types';
...
/**
 * Create a Web MIDI adapter implementing D110MidiIO interface
 *
 ...
 * @returns D110MidiIO adapter
 */
export function createWebMidiAdapter(input: MIDIInput, output: MIDIOutput): D110MidiIO {
  ...
  return {
    send(message: number[]): void {
      console.log(
        '[WebMIDI] Sending:',
        message.map((b) => b.toString(16).padStart(2, '0')).join(' ')
      );
 ...
 * @returns Promise resolving to D110MidiIO adapter and cleanup function
 */
export async function openMidiPorts(
  inputId: string,
  outputId: string
): Promise<{ adapter: D110MidiIO; cleanup: () => Promise<void> }> {
  if (!isWebMidiSupported()) {
