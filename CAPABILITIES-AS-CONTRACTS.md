...
**The test-name protocol.** All 146 UI tests in the editor's capability suite start with the capability ID they verify. Examples: `test('D-PATCH-01: clicking a patch slot selects it', ...)`, `test('D-TONE-WAVE-09: changing the wave bank emits the correct DT1 SysEx', ...)`, `test('D-LIB-08: dropping a library tone onto a device slot mounts ImportLibraryToneDialog', ...)`. The protocol made coverage auditable: a grep `grep -rn "test('D-" test/wiring/` returns every capability-bound test, and a script cross-references the inventory to find any `implemented` or `partial` row without a citation. Phase 0 Task 10's acceptance gate was *"every `implemented` or `partial` capability has a non-empty test citation"* — a grep-checkable condition that the team could close decisively.
